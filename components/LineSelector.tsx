...
export const LineSelector: React.FC<LineSelectorProps> = ({ lines, onSelectLine, onAddLine, onDeleteLine }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-64 bg-brand-900 shadow-xl z-0"></div>
      <div className="absolute top-0 right-0 w-1/3 h-64 bg-brand-800 opacity-50 skew-x-12 z-0"></div>
      ...
      <div className="relative z-10 max-w-6xl w-full mx-auto px-6 pt-16 pb-12 flex-1 flex flex-col">
        ...
        <div className="flex justify-between items-end mb-10 text-white">
          <div>
            <h1 className="text-lg font-semibold text-brand-200 uppercase tracking-widest mb-2">Plataforma</h1>
            <h2 className="text-4xl font-bold tracking-tight text-white">Gestión de Líneas Ius</h2>
            <p className="text-brand-200 mt-2 max-w-xl text-lg font-light">
              Gestión estratégica de proyectos y despachos internacionales.
            </p>
          </div>
          <button
            onClick={() => setIsAdding(true)}
            className="group flex items-center space-x-2 bg-white text-brand-900 px-6 py-3 rounded-lg font-semibold hover:bg-brand-50 transition-all shadow-lg active:scale-95"
          >
            <Plus className="h-5 w-5 text-accent-600 group-hover:rotate-90 transition-transform duration-300" />
            ...
            <div className="overflow-y-auto flex-1">
                {lines.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full p-12 text-center">
                        ...
                        <button 
                            onClick={() => setIsAdding(true)}
                            className="mt-6 text-accent-600 hover:text-accent-700 font-medium hover:underline"
                        >
                    ...
                    lines.map((line) => (
                        <div 
                            ...
                            className="group grid grid-cols-12 gap-4 px-8 py-5 border-b border-slate-100 items-center hover:bg-brand-50/50 cursor-pointer transition-colors"
                        >
                            <div className="col-span-12 md:col-span-4 flex items-center min-w-0">
                                <div className="p-2 bg-brand-100 text-brand-700 rounded-lg mr-4 group-hover:bg-brand-200 transition-colors">
                                    <FolderOpen className="h-5 w-5" />
                                </div>
                                <div className="truncate">
                                    <h3 className="text-base font-bold text-slate-800 group-hover:text-brand-700 transition-colors truncate">{line.name}</h3>
                            ...
                            <div className="col-span-12 md:col-span-1 flex items-center justify-end space-x-2">
                                ...
                                <div className="p-2 text-slate-300 group-hover:text-brand-600 transition-colors">
            ...
            <form onSubmit={handleCreate} className="p-8 space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 py-3 px-4 text-slate-900 placeholder:text-slate-400 border bg-white"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 py-3 px-4 text-slate-900 placeholder:text-slate-400 border bg-white resize-none"
              ...
              <div className="flex justify-end space-x-3 pt-2">
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-brand-800 hover:bg-brand-900 text-white rounded-lg font-medium shadow-md transition-all active:scale-95"
                >
                  Crear Línea
                </button>
              </div>
