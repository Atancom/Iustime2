...
export const Projects: React.FC<ProjectsProps> = ({ projects, lineId, onAddProject, onUpdateProject, onDeleteProject }) => {
  ...
  const getStatusBadge = (status: Project['status']) => {
    const styles = {
      ...
      'Ready to Start': 'bg-slate-100 text-slate-600 border-slate-200',
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-end border-b border-slate-200 pb-6">
        <div>
            <h2 className="text-3xl font-bold text-slate-800 tracking-tight">Proyectos</h2>
            <p className="text-slate-500 mt-2">Visión global de iniciativas y estado de ejecución.</p>
        </div>
        <button
          onClick={() => {
        ...
        <div className="bg-slate-50 p-8 rounded-xl shadow-inner border border-slate-200 mb-8">
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="lg:col-span-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border-slate-200 shadow-sm focus:border-brand-500 focus:ring-brand-500 p-3 border bg-white text-slate-900 placeholder:text-slate-400"
                  />
                </div>
                <div className="lg:col-span-2">
                  ...
                  <textarea
                    ...
                    className="w-full rounded-lg border-slate-200 shadow-sm focus:border-brand-500 focus:ring-brand-500 p-3 border bg-white text-slate-900 placeholder:text-slate-400 resize-y"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border-slate-200 shadow-sm focus:border-brand-500 focus:ring-brand-500 p-3 border bg-white text-slate-900"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Dificultad</label>
                  <select
                 ...
                 <div className="lg:col-span-2">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-sm font-bold text-accent-600 bg-accent-50 px-2 py-0.5 rounded">{newProject.progress || 0}%</span>
                  </div>
                  <div className="relative pt-1 px-1">
                     <input
                      ...
                      className="w-full h-3 bg-slate-200 rounded-lg appearance-none cursor-not-allowed accent-slate-400 opacity-70"
                      title="Calculado automáticamente según el progreso de las tareas"
                    />
                     <div className="flex justify-between text-[10px] text-slate-400 mt-1 select-none font-medium px-0.5">
                        <span>0%</span>
                        <span>20%</span>
                        <span>40%</span>
                        <span>60%</span>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Fin Estimado</label>
                  <input
                    type="date"
                ...
                <div className="lg:col-span-2">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Próximos pasos (Lista)</label>
                  <div className="flex gap-2 mb-2">
                    <input
                      ...
                      className="flex-1 rounded-lg border-slate-200 shadow-sm focus:border-brand-500 focus:ring-brand-500 p-3 border bg-white text-slate-900 placeholder:text-slate-400"
                    />
                    <button 
                      type="button" 
                      onClick={handleAddStep}
                      className="bg-brand-100 text-brand-700 p-3 rounded-lg hover:bg-brand-200 transition-colors"
                    >
                  ...
                  <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
                    {newProject.nextSteps?.map((step, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-white border border-slate-100 p-2 rounded text-sm text-slate-700">
                        <div className="flex items-start">
                           <ListPlus className="h-4 w-4 mr-2 text-brand-400 mt-0.5" />
                           <span>{step}</span>
                        </div>
                        <button 
                          type="button" 
                    ...
                    {(!newProject.nextSteps || newProject.nextSteps.length === 0) && (
                      <div className="text-xs text-slate-400 italic p-1">No hay pasos añadidos</div>
                    )}
                  </div>
                </div>
                
                <div className="lg:col-span-2">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Notas</label>
                  <textarea
                    rows={4}
              ...
              <div className="flex justify-end border-t border-slate-200 pt-6">
                <button
                  type="submit"
                  className="bg-brand-800 hover:bg-brand-900 text-white px-8 py-3 rounded-lg shadow-md transition-colors font-medium flex items-center"
                >
          ...
          <table className="min-w-full divide-y divide-slate-100">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider min-w-[180px]">Proyecto</th>
                <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider min-w-[200px]">Objetivo</th>
                <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Responsable</th>
                <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Fechas</th>
                <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Estado / Progreso</th>
                <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Prioridad</th>
                <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Dificultad</th>
                <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider min-w-[200px]">Siguientes Pasos</th>
                ...
                <tr>
                  <td colSpan={9} className="px-6 py-16 text-center text-slate-400">
                    <div className="flex flex-col items-center justify-center opacity-60">
                        ...
                        <p className="text-lg font-medium text-slate-600">No hay proyectos activos</p>
                ...
                projects.map((project) => (
                  ...
                  >
                    <td className="px-6 py-5 whitespace-nowrap">
                        ...
                        {project.notes && (
                          <div className="text-xs text-slate-400 mt-1 line-clamp-2 max-w-[150px]" title={project.notes}>
                            {project.notes}
                          </div>
                        )}
                    </td>
                    <td className="px-6 py-5 text-sm text-slate-600">
                        <div className="whitespace-pre-wrap line-clamp-3" title={project.objective}>{project.objective || <span className="text-slate-300 italic">No definido</span>}</div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-slate-600">
                        <div className="flex items-center">
                            <div className="h-7 w-7 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center text-xs font-bold text-slate-600 mr-2.5 border border-white shadow-sm">
                                {(project.assignee || 'U').charAt(0).toUpperCase()}
                            </div>
                            <span className="font-medium text-slate-700">{project.assignee || 'Sin asignar'}</span>
                        </div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap">
                        <div className="text-xs text-slate-500 flex flex-col gap-1">
                            <span className="flex items-center"><span className="w-10 font-medium text-slate-400">Inicio:</span> {project.startDate}</span>
                            <span className="flex items-center"><span className="w-10 font-medium text-slate-400">Fin:</span> {project.endDate}</span>
                        </div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap">
                        <div className="flex flex-col gap-2">
                             ...
                             <div className="flex items-center gap-2">
                                <div className="w-20 bg-slate-100 rounded-full h-1.5 overflow-hidden border border-slate-200">
                                    <div 
                                    className={`h-1.5 rounded-full ${project.status === 'Delayed' ? 'bg-rose-500' : 'bg-accent-500'}`} 
                          ...
                          {Array.isArray(project.nextSteps) && project.nextSteps.length > 0 ? (
                            project.nextSteps.map((step, i) => (
                              <li key={i} className="flex items-start text-xs text-slate-600">
                                <ArrowRight className="h-3 w-3 mr-1.5 mt-0.5 text-brand-400 flex-shrink-0" />
                                <span className="line-clamp-2">{step}</span>
                              </li>
                            ))
                          ) : (
                            <li className="text-xs text-slate-300 italic">Sin acciones pendientes</li>
                          )}
                        </ul>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-1">
                        <button 
                          ...
                          className="text-slate-400 hover:text-brand-600 hover:bg-brand-50 transition-all p-2 rounded-full"
                        ...
                        <button 
                          ...
                          className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all p-2 rounded-full"
