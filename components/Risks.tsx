...
export const Risks: React.FC<RisksProps> = ({ risks, tasks, lineId, onAddRisk, onUpdateRisk, onDeleteRisk }) => {
  ...
  const [impact, setImpact] = useState<Risk['impact']>('Low'); // Keep impact as distinct from Priority if needed, or just keep it for sorting
  ...
  const getStatusBadge = (s: Risk['status']) => {
      switch(s) {
          case 'Open': return <span className="text-xs font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded">Abierto</span>;
          ...
          case 'Closed': return <span className="text-xs font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded line-through">Cerrado</span>;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
            <h2 className="text-3xl font-bold text-slate-800 tracking-tight">Gestión de Riesgos</h2>
            <p className="text-slate-500 mt-2">Identificación y plan de acción para contingencias.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
          ...
          <div className="lg:col-span-2">
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Vincular a Tarea / Subtarea</label>
            <div className="relative">
                <Link className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                <select
                    value={taskId}
                    onChange={(e) => setTaskId(e.target.value)}
                    className="w-full rounded-md border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 p-2 pl-9 border text-sm"
                >
          ...
          <div className="lg:col-span-2">
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Descripción del Riesgo</label>
            <input
              type="text"
              value={description}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Impacto (Severidad)</label>
            <select
              value={impact}
           ...
           <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Prioridad de Acción</label>
            <select
              value={priority}
          ...
          <div className="lg:col-span-4">
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Acción Requerida / Mitigación</label>
            <textarea
              rows={2}
              value={requiredAction}
              onChange={(e) => setRequiredAction(e.target.value)}
              className="w-full rounded-md border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 p-2 border text-sm resize-y"
      ...
      <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
        <div className="overflow-x-auto">
            ...
            <thead className="bg-slate-50">
                <tr>
                <th className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Tarea Vinculada</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Acción Requerida</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Impacto</th>
            ...
            <tbody className="bg-white divide-y divide-slate-200">
                ...
                risks.map((risk) => (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{risk.responsible || '--'}</td>
