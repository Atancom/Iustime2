...
const App: React.FC = () => {
  ...
  const [lines, setLines] = useState<WorkLine[]>(() => {
    try {
        ...
        return Array.isArray(parsed) && parsed.length > 0 ? parsed : [{ id: 'line-1', name: 'Línea General', description: 'Línea por defecto', createdAt: new Date().toISOString() }];
    } catch {
        return [{ id: 'line-1', name: 'Línea General', description: 'Línea por defecto', createdAt: new Date().toISOString() }];
    }
  });

  const [projects, setProjects] = useState<Project[]>(() => {
    try { return JSON.parse(localStorage.getItem('iustime_projects') || '[]'); } catch { return []; }
  });
  
  const [tasks, setTasks] = useState<Task[]>(() => {
    try { return JSON.parse(localStorage.getItem('iustime_tasks') || '[]'); } catch { return []; }
  });
  
  const [risks, setRisks] = useState<Risk[]>(() => {
    try { return JSON.parse(localStorage.getItem('iustime_risks') || '[]'); } catch { return []; }
  });

  // Self-healing user state: Always ensure Admin exists
  const [users, setUsers] = useState<User[]>(() => {
    ...
    try {
        const saved = localStorage.getItem('iustime_users');
        if (!saved) return defaultUsers;
        
        const parsedUsers = JSON.parse(saved);
        if (!Array.isArray(parsedUsers) || parsedUsers.length === 0) return defaultUsers;
        
        // Ensure admin exists in the loaded data
        if (!parsedUsers.some((u: User) => u.email === 'admin@iustime.com')) {
            return [...parsedUsers, defaultUsers[0]];
        }
        return parsedUsers;
    } catch {
        return defaultUsers;
